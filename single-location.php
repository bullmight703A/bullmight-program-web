...
while (have_posts()):
	...
	// Proactively strip out raw LLM AI prompts that were pasted into the WYSIWYG editor
	if (strpos($raw_content, 'SEO Input Prompt') !== false) {
		$raw_content = substr($raw_content, 0, strpos($raw_content, 'SEO Input Prompt'));
	}
	// Also filter the SEO Content field if it was stuffed
	$seo_content_text = get_post_meta($location_id, 'location_seo_content_text', true);
	if (strpos($seo_content_text, 'SEO Input Prompt') !== false) {
		$seo_content_text = substr($seo_content_text, 0, strpos($seo_content_text, 'SEO Input Prompt'));
	}
	...
	// SEO content (already fetched above with AI strip logic)
			...
			<div class="grid lg:grid-cols-3 gap-12">
				<div class="lg:col-span-2 space-y-12">
					...
					<section>
						...
						<div class="text-slate-600 leading-relaxed text-lg mb-8 content-filtered">
				...
				<section class="mt-24">
					...
					<div class="grid md:grid-cols-3 gap-8">
						<?php while ($programs_query->have_posts()): $programs_query->the_post(); ?>
							<a href="<?php the_permalink(); ?>" class="bg-white p-6 rounded-[2rem] border border-brand-ink/5 hover:border-kidazzle-red/30 transition-all group shadow-sm hover:shadow-xl flex flex-col h-full">
								<?php if (has_post_thumbnail()): ?>
									<div class="w-full h-32 mb-4 rounded-xl overflow-hidden group-hover:scale-105 transition-transform duration-300">
